...
export const register = catchAsyncErrors(async(req,res,next)=>{
   try{
      ...
      if(req.files && req.files.resume){
         ...
         if(resume){
            try{
               const cloudinaryResposnse = await cloudinary.uploader.upload(resume.tempFilePath,{folder:"Job_Seeker_Resume"}

               );
               if(!cloudinaryResposnse || cloudinaryResposnse.error){
                  return next(
                     new ErrorHandler("Failed to upload resume to cloud.", 500)
                  );
               }
               userData.resume = {
                  public_id: cloudinaryResposnse.public_id,
                  url:cloudinaryResposnse.secure_url,
               };
   ...
   const newUserData = {
      ...
      if(req.files){
         ...
         if(resume){
            ...
            const newResume = await cloudinary.uploader.upload(resume.tempFilePath,{folder:"Job_Seeker_Resume"}
            );
            newUserData.resume = {
